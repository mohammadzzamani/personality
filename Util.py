...
def evaluate(Ytrue, Ypred, type='regression',  pre = 'pre ', store=True, scaler=None):
    ...
    mse = mean_squared_error(Ytrue,Ypred)
    # print ('mae: '  , mae)
    with open("res_county_disease_sep_adapted.txt", "a") as myfile:
        if type is 'regression':
            # print ('type: ' , type)
...
def split_train_test(groupData, groupLabels, foldsdf, fold, dim_sizes = None):
    ...
    for i in range(len(groupData)):
        ...
        if dim_sizes is not None:
            print ('i , dim_size ....... ' ,i, ' , ',  dim_sizes[i],' , ', Xtrain.shape, ' , ', ytrain.shape)
...
def dimension_reduction(X, y, univariate = True, pca = True, dim_size = (300, 60.0)):
    print ('dimension_reduction...')
