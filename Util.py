...
def dimension_reduction(X, y, univariate = True, pca = True):
    ...
    if univariate & pca:
        featureSelectionString = 'Pipeline([ ("1_univariate_select", SelectFwe(f_regression, alpha='+str(alpha)+')), ("2_rpca", PCA(n_components=min('+str(n_components)+',X.shape[1]), random_state=42, whiten=False, iterated_power=3))])'
    elif univariate:
        featureSelectionString = 'Pipeline([ ("1_univariate_select", SelectFwe(f_regression, alpha='+str(alpha)+'))])'
    elif pca:
        featureSelectionString = 'Pipeline([ ("1_rpca", PCA(n_components=min('+str(n_components)+',X.shape[1]),, random_state=42, whiten=False, iterated_power=3))])'
